...
DHT (Distributed hash table, распределенная хэш-таблица) — это протокол, позволяющий битторрент-клиентам находить друг друга без использования трекера. Похожую задачу выполняет протокол PEX (Peer exchange, обмен пиров), который позволяет участникам роя обмениваться списками пиров друг друга для этой раздачи.

Преследует следующие задачи:

*	Помогает пирам быстрее находить друг друга;
*	Снижает нагрузку на трекер;
*	Позволяет поддерживать раздачи при временной неработоспособности трекера или вообще его отсутствии.`,
		markdown:true,
		name:'DHT',
		id:'DHT'
	}
